...
#### MAVD_TMM simulation ####
folder_path <- "/public/linzhf/simulation_data/"
output_folder <- "/public/linzhf/simulation_output/TMM"
# Get the list of RData files in the folder
file_list <- list.files(path = folder_path, pattern = ".RData", full.names = TRUE)
# Iterate over each RData file
for (file_path in file_list) {
  tryCatch({
    load(file_path) # Load the data from the current file
    sim <- res_MAVD_TMM(sim)  # Apply the new_cut function
    file_name <- tools::file_path_sans_ext(basename(file_path)) # Extract the file name without extension
    save(sim, file = file.path(output_folder, paste0(file_name, ".RData")))
  },  error = function(e){cat(paste0("error in ",file_path))} )
}
...
#### MAVD_RLE simulation ####
folder_path <- "/public/linzhf/simulation_data/"
output_folder <- "/public/linzhf/simulation_output/RLE"
# Get the list of RData files in the folder
file_list <- list.files(path = folder_path, pattern = ".RData", full.names = TRUE)
# Iterate over each RData file
for (file_path in file_list) {
  tryCatch({
    load(file_path) # Load the data from the current file
    sim <- res_MAVD_RLE(sim)  # Apply the new_cut function
    file_name <- tools::file_path_sans_ext(basename(file_path)) # Extract the file name without extension
    save(sim, file = file.path(output_folder, paste0(file_name, ".RData")))
  },  error = function(e){cat(paste0("error in ",file_path))} )
}

#### MAVD_GMPR simulation ####
folder_path <- "/public/linzhf/simulation_data/"
output_folder <- "/public/linzhf/simulation_output/GMPR"
# Get the list of RData files in the folder
file_list <- list.files(path = folder_path, pattern = ".RData", full.names = TRUE)
# Iterate over each RData file
for (file_path in file_list) {
  tryCatch({
    load(file_path) # Load the data from the current file
    sim <- res_MAVD_GMPR(sim)  # Apply the new_cut function
    file_name <- tools::file_path_sans_ext(basename(file_path)) # Extract the file name without extension
    save(sim, file = file.path(output_folder, paste0(file_name, ".RData")))
  },  error = function(e){cat(paste0("error in ",file_path))} )
}
...
#### MAVD_ALR simulation####
folder_path <- "/public/linzhf/simulation_data/"
output_folder <- "/public/linzhf/simulation_output/ALR"
# Get the list of RData files in the folder
file_list <- list.files(path = folder_path, pattern = ".RData", full.names = TRUE)
# Iterate over each RData file
for (file_path in file_list) {
  tryCatch({
    load(file_path) # Load the data from the current file
    sim <- res_MAVD_ALR(sim)  # Apply the new_cut function
    file_name <- tools::file_path_sans_ext(basename(file_path)) # Extract the file name without extension
    save(sim, file = file.path(output_folder, paste0(file_name, ".RData")))
  },  error = function(e){cat(paste0("error in ",file_path))} )
}
